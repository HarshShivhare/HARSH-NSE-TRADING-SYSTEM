...
from app.opportunity_entry_discovery import run_opportunity_entry_discovery, write_opportunity_entry_reports
...
def cmd_opportunity_entry(args):
    files = _matched_files(args.data_glob)
    if not files:
        raise SystemExit(f"No files matched: {args.data_glob}")
    cfg = _strategy_config_from_args(args)
    result = run_opportunity_entry_discovery(
        files, cfg, args.dev_start, args.dev_end, args.validation_start, args.validation_end, args.bootstrap_samples
    )
    print("\n=== V20 OPPORTUNITY RANK + ENTRY DISCOVERY — DEV + VALIDATION ONLY ===")
    print(f"ranked candidate rows : {len(result.candidates)}")
    print(f"entry evaluation rows : {len(result.entries)}")
    if not result.entry_summary.empty:
        print("\n--- ENTRY STRUCTURE SUMMARY (60M / 120M) ---")
        show = result.entry_summary[result.entry_summary.horizon_min.isin([60,120])]
        cols = ["split","score_family","top_k","snapshot_time","entry_variant","horizon_min","candidates","triggered","trigger_rate","avg_fwd_atr","median_fwd_atr","positive_rate","avg_mfe_atr","avg_mae_atr","avg_entry_delay_min","session_ci_low","session_ci_high"]
        print(show[cols].to_string(index=False))
    if not result.paired_improvement.empty:
        print("\n--- MATCHED ENTRY IMPROVEMENT VS IMMEDIATE (60M / 120M) ---")
        show = result.paired_improvement[result.paired_improvement.horizon_min.isin([60,120])]
        print(show.to_string(index=False))
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_dir = Path(args.report_dir) / f"opportunity_entry_{stamp}"
    paths = write_opportunity_entry_reports(result, report_dir)
    print(f"\nV20 reports: {report_dir.resolve()}")
    for name, path in paths.items(): print(f"  {name:22s}: {path.name}")
    print("2026 FINAL OOS was NOT evaluated. V20 is entry discovery only; no capital is allocated.")
    print("Do not promote a pattern merely because one timestamp/split is strongest; require broad DEV + Validation improvement over immediate entry.")
...
def build_parser():
    ...
    p=sub.add_parser("opportunity-entry", help="V20 causal Top1/3 opportunity ranking + entry-structure discovery")
    p.add_argument("--data-glob",default="data/NSE_*_5minute_*.parquet"); p.add_argument("--report-dir",default="reports")
    p.add_argument("--dev-start",default="2023-09-01"); p.add_argument("--dev-end",default="2025-06-30")
    p.add_argument("--validation-start",default="2025-07-01"); p.add_argument("--validation-end",default="2025-12-31")
    p.add_argument("--bootstrap-samples",type=int,default=1000); add_strategy_args(p,True)
    p.set_defaults(func=cmd_opportunity_entry)
